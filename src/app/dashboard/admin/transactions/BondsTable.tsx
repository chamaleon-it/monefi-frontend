...
function BondsRow({ tx, mutate }: { tx: Transaction; mutate: () => void }) {
  ...
  return (
    <>
      <tr className="hover:bg-slate-50/80 transition-colors whitespace-nowrap">
        ...
        <td className="py-4 px-5 text-xs font-mono text-slate-600 bg-slate-50 px-2 py-0.5 rounded border border-slate-200 inline-block my-3">{tx.symbol}</td>
          ...
          <table className="w-full whitespace-nowrap text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!isLoading && transactions.length > 0 && (
                <>
                  {transactions.map((tx) => (
                    <BondsRow key={tx._id} tx={tx} mutate={mutate} />
                  ))}
                </>
              )}
