...
export default function StepTrustDetails({ formData, updateFormData, onNext, onBack }: StepProps) {
  ...
  const handleChange = (field: string, value: string) => {
    updateFormData('trustDetails', { ...details, [field]: value });
    if (errors[field]) setErrors((prev) => ({ ...prev, [field]: '' }));
  };

  const validateAndNext = () => {
    const newErrors: Record<string, string> = {};
    if (!details.trustName.trim()) newErrors.trustName = 'Trust name is required';
    if (!details.trustType.trim()) newErrors.trustType = 'Trust type is required';
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 sm:px-8 lg:px-10 py-10 sm:py-14 font-inter w-full">
      <div className="mb-10">
        ...
        <h1 className="text-[1.875rem] sm:text-[2.125rem] font-semibold text-slate-900 tracking-tight leading-tight mb-2">Trust details</h1>
        <p className="text-[15px] text-slate-600 leading-relaxed">Provide basic information about the trust.</p>
      </div>
      ...
      <div className="space-y-5">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-[0_2px_20px_rgba(0,0,0,0.06)] p-8 sm:p-10">
          <div className="flex items-start gap-4 mb-8 pb-6 border-b border-slate-100">
            ...
            <div>
              <p className="text-[17px] font-semibold text-slate-900">Trust Information</p>
              <p className="text-[13px] text-slate-500 mt-1">Official registered details of the trust</p>
            </div>
          </div>

          <div className="space-y-5">
            <div>
              <label className={LABEL}>Name of Trust *</label>
              <input type="text" value={details.trustName} onChange={e => handleChange('trustName', e.target.value)} className={ic(errors.trustName)} />
              {errors.trustName && <p className={ERROR_MSG}><AlertCircle className="w-3.5 h-3.5" strokeWidth={1.5} />{errors.trustName}</p>}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <label className={LABEL}>Type of Trust *</label>
                <input type="text" placeholder="e.g. Discretionary" value={details.trustType} onChange={e => handleChange('trustType', e.target.value)} className={ic(errors.trustType)} />
                {errors.trustType && <p className={ERROR_MSG}><AlertCircle className="w-3.5 h-3.5" strokeWidth={1.5} />{errors.trustType}</p>}
              </div>
              <div>
                <label className={LABEL}>Country Established *</label>
                <input type="text" value={details.countryEstablished} onChange={e => handleChange('countryEstablished', e.target.value)} className={ic(errors.countryEstablished)} />
              </div>
            </div>
            <div>
              <label className={LABEL}>Nature of Trust *</label>
              <input type="text" value={details.natureOfTrust} onChange={e => handleChange('natureOfTrust', e.target.value)} className={ic(errors.natureOfTrust)} />
            </div>
          </div>
        </div>
      </div>
